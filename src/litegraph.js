...
(function (global) {
  ...
  var LiteGraph = global.LiteGraph = {
    ...
    registerNodeType: function registerNodeType(type, base_class) {
      ...
      if (!Object.prototype.hasOwnProperty.call(base_class.prototype, "shape")) {
        ...
        if (base_class.supported_extensions) {
          for (var _i in base_class.supported_extensions) {
            var ext = base_class.supported_extensions[_i];
     ...
     */
    unregisterNodeType: function unregisterNodeType(type) {
    ...
    */
    registerNodeAndSlotType: function registerNodeAndSlotType(type, slot_type, out) {
      out = out || false;
     ...
     */
    buildNodeClassFromObject: function buildNodeClassFromObject(name, object) {
      var ctor_code = "";
     ...
     */
    wrapFunctionAsNode: function wrapFunctionAsNode(name, func, param_types, return_type, properties) {
     ...
     */
    clearRegisteredTypes: function clearRegisteredTypes() {
     ...
     * @param {Function} func
     */
    addNodeMethod: function addNodeMethod(name, func) {
    ...
    createNode: function createNode(type, title, options) {
     ...
     */
    getNodeType: function getNodeType(type) {
    ...
    getNodeTypesInCategory: function getNodeTypesInCategory(category, filter) {
      var r = [];
      for (var i in this.registered_node_types) {
        var type = this.registered_node_types[i];
     ...
     */
    getNodeTypesCategories: function getNodeTypesCategories(filter) {
      var categories = {
        "": 1
      };
      for (var i in this.registered_node_types) {
    ...
    reloadNodes: function reloadNodes(folder_wildcard) {
    ...
    cloneObject: function cloneObject(obj, target) {
      if (obj == null) {
        return null;
      }
     ...
     */
    isValidConnection: function isValidConnection(type_a, type_b) {
     ...
     * @return {Boolean} true if they can be connected
     */
    registerSearchboxExtra: function registerSearchboxExtra(node_type, description, data) {
     ...
     */
    fetchFile: function fetchFile(url, type, on_complete, on_error) {
      var that = this;
      if (!url) return null;
      type = type || "text";
  ...
  * supported callbacks:
  ...
  LGraph.prototype.start = function (interval) {
    ...
    if (interval == 0 && typeof window != "undefined" && window.requestAnimationFrame) {
      var on_frame = function on_frame() {
  ...
  LGraph.prototype.arrange = function (margin, layout) {
    ...
    for (var _i2 = 0; _i2 < columns.length; ++_i2) {
      ...
      for (var j = 0; j < column.length; ++j) {
        var _node = column[j];
        _node.pos[0] = layout == LiteGraph.VERTICAL_LAYOUT ? y : x;
        _node.pos[1] = layout == LiteGraph.VERTICAL_LAYOUT ? x : y;
        var max_size_index = layout == LiteGraph.VERTICAL_LAYOUT ? 1 : 0;
        if (_node.size[max_size_index] > max_size) {
          max_size = _node.size[max_size_index];
        }
        var node_size_index = layout == LiteGraph.VERTICAL_LAYOUT ? 0 : 1;
        y += _node.size[node_size_index] + margin + LiteGraph.NODE_TITLE_HEIGHT;
      }
      x += max_size + margin;
    }
  ...
  */
  ...
  LGraphCanvas.prototype.processNodeWidgets = function (node, pos, event, active_widget) {
    ...
    var _loop = function _loop() {
        ...
        widget_height = w.computeSize ? w.computeSize(width)[1] : LiteGraph.NODE_WIDGET_HEIGHT;
        ...
        switch (w.type) {
          ...
          case "combo":
            ...
            if (event.type == LiteGraph.pointerevents_method + "move" && w.type == "number") {
              ...
              if (w.type == "number") {
                ...
                index = -1;
                _this.last_mouseclick = 0; //avoids dobl click event
                ...
                var inner_clicked = function inner_clicked(v, option, event) {
                ...
                menu = new LiteGraph.ContextMenu(text_values, {
                  scale: Math.max(1, _this.ds.scale),
                  event: event,
                  className: "dark",
              ...
              if (event.click_time < 200 && delta == 0) {
                _this.prompt("Value", w.value, function (v) {
          ...
          default:
            if (w.mouse) {
              _this.dirty_canvas = w.mouse(event, [x, y], node);
            }
            break;
        } //end switch
  ...
  LGraphCanvas.alignNodes = function (nodes, direction, align_to) {
    ...
    for (var _i3 = 0, _Object$entries = Object.entries(canvas.selected_nodes); _i3 < _Object$entries.length; _i3++) {
  ...
  LGraphCanvas.onMenuAdd = function (node, options, e, prev_menu, _callback) {
    var canvas = LGraphCanvas.active_canvas;
    var ref_window = canvas.getCanvasWindow();
  ...
  LGraphCanvas.prototype.createDefaultNodeForSlot = function (optPass) {
    ...
    if (slotTypesDefault && slotTypesDefault[fromSlotType]) {
      ...
      nodeNewType = false;
      if (_typeof(slotTypesDefault[fromSlotType]) == "object" || typeof slotTypesDefault[fromSlotType] == "array") {
      ...
      if (nodeNewType) {
        ...
        if ((typeof nodeNewType === "undefined" ? "undefined" : _typeof(nodeNewType)) == "object" && nodeNewType.node) {
    ...
    */
    ...
    function refreshHelper() {
      ...
      if (that.onSearchBox) {
        ...
        var inner_test_filter = function inner_test_filter(type, optsIn) {
          var optsIn = optsIn || {};
          var optsDef = {
            skipFilter: false,
  ...
  function ContextMenu(values, options) {
    ...
    LiteGraph.pointerListenerAdd(root, "up", function (e) {
    ...
    LiteGraph.pointerListenerAdd(root, "down", function (e) {
  ...
  CurveEditor.prototype.draw = function (ctx, size, graphcanvas, background_color, line_color, inactive) {
    ...
    ctx.stroke();
    ctx.globalAlpha = 1;
    if (!inactive) for (var i = 0; i < points.length; ++i) {
      var p = points[i];
  ...
  LiteGraph.pointerListenerAdd = function (oDOM, sEvIn, fCall) {
  ...
  LiteGraph.pointerListenerRemove = function (oDOM, sEvent, fCall) {
    var capture = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
